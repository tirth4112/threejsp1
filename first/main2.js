...
function loadModelWithCacheBusting(modelPath, scene) {
    ...
    // Assuming you have an FBXLoader available in your code

    loader.load(cacheBustingPath, (fbx) => {
        // Adjust the position, scale, or rotation of the loaded FBX model if needed
        fbx.position.set(0, 10, 20);
        // fbx.scale.set(0.1, 0.1, 0.1); // Example of scaling
        fbx.rotation.set(0, Math.PI, 0); // Example of rotation

        // Add the FBX model to the scene
        scene.add(fbx);
    });
}
loadModelWithCacheBusting('/onj/4/little_boy_B.fbx', scene1);
loadModelWithCacheBusting('/onj/4/casual_Female_G.fbx', scene2);
loadModelWithCacheBusting('/onj/4/casual_Male_G.fbx', scene3);

loadModelWithCacheBusting('/onj/4/Doctor_Male_B.fbx', scene4);
loadModelWithCacheBusting('/onj/4/people_freePack_webGl_ani.fbx', scene5);
loadModelWithCacheBusting('/onj/4/police_Female_A.fbx', scene6);
